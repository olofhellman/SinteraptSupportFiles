...
to makeMassRanges()
	tell application "Sinterapt"
		tell document 1
			...
			make new mass range with properties {lower bound:57, upper bound:62.6, atom list:{"Ni"}, name:"Ni+"}
			make new mass range with properties {lower bound:62.7, upper bound:63.6, atom list:{"Cu"}, name:"Cu+"}
			make new mass range with properties {lower bound:63.7, upper bound:64.6, atom list:{"Ni"}, name:"Ni+"}
			make new mass range with properties {lower bound:64.7, upper bound:66.6, atom list:{"Cu"}, name:"Cu+"}
		end tell
	end tell
end makeMassRanges
...
my makeMassRanges()
my setAtomtypeProps("Ni", {1.0, 0.2, 0.5}, 1, true)
my setAtomtypeProps("Cr", {0.3, 1.0, 0.5}, 1, true)
my setAtomtypeProps("Cu", {0.5, 0.5, 1.0}, 1, true)
